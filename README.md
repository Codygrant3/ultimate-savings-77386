...
- Compares parsed candidates with tracked offers using normalized merchants, strong offer-specific language, and compatible reward types, then flags differences in captured value, minimum spend, or expiration so review time focuses on evidence changes rather than unrelated same-brand offers.
...
- Honors captured official day-of-week terms in weekly planning, so recurring Wednesday, Friday, Sunday, weekday, or midweek offers cannot be recommended on an unsupported day while stated time windows still require confirmation.
- Honors captured official clock windows in weekly planning: selected deals disclose an upcoming or active window, a closed window is excluded for that day, and malformed timing evidence fails closed.
...
- Tests a bounded one-step redemption-effort relaxation and reports it only when risk-adjusted planning value improves, keeping effort visible as an explicit household tradeoff rather than silently loosening the gate.
- Discloses when an effort tradeoff lowers the official estimate but is recommended only because linked local outcomes produce a stronger risk-adjusted household result.
